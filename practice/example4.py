...
cv.setTrackbarPos('threshold','img_result',30) #맨 뒤에 변수는 초기값
cap=cv.VideoCapture("http://192.168.0.24:7500/?action=stream")
...
while(True):
    ...
    img_mask = cv.morphologyEx(img_mask, cv.MORPH_CLOSE,kernel) #검은색 구멍 제거
    ...
    #물체의 좌표 추적하는 코드
    nunLabel,ing_label,stats,centroids=cv.connectedComponentsWithStats(img_mask) #쉽게 라벨링 하는 함수
